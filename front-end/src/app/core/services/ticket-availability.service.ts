...
export class TicketAvailabilityService {
    ...
    private availabilitySubject = new BehaviorSubject<{
        [eventId: number]: TicketAvailability
    }>({})
    ...
    getEventAvailability(
        eventId: number
    ): Observable<TicketAvailability | null> {
        return this.availability$.pipe(
            distinctUntilChanged(),
            filter(
                availability =>
                    availability.hasOwnProperty(eventId) ||
    ...
    isEventAlmostSoldOut(eventId: number): Observable<boolean> {
        return this.getEventAvailability(eventId).pipe(
            switchMap(availability =>
                of(availability?.isAlmostSoldOut || false)
            )
        )
    }
    ...
    getAvailabilityStatus(
        eventId: number
    ): Observable<{ text: string; class: string; disabled: boolean }> {
        return this.getEventAvailability(eventId).pipe(
            switchMap(availability => {
                if (!availability) {
                    return of({
    ...
    private fetchApiAvailability(eventIds: number[]): void {
        this.apiService
            .getMultipleEventsAvailability(eventIds)
            .pipe(
                catchError(error => {
                    console.error(
                        'Error fetching availability from API:',
                        error
                    )
                    return of(null)
                })
                    ...
                    Object.keys(response.events).forEach(eventIdStr => {
                        ...
                        availability[eventId] = {
                            ...
                            lastUpdated: new Date(
                                eventData.last_updated || Date.now()
                            ),
                        }
                    })
    ...
    private fetchMockAvailability(eventIds: number[]): void {
        ...
        eventIds.forEach(eventId => {
            ...
            const bookedTickets = Math.floor(
                baseCapacity * randomBookedPercentage
            )
